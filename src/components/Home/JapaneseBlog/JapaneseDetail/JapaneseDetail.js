...
const JapaneseDetail = () => {
    ...
    useEffect(()=>{
        fetch('https://intense-waters-45078.herokuapp.com/japaneseblog')
        .then(res => res.json())
    ...
    const handleForm = e =>{
        ...
        fetch('https://intense-waters-45078.herokuapp.com/comments',{
            method:'POST',
            headers:{
    ...
    const deleteComments = id =>{
        const procude = window.confirm('are you sure,you want to delete this comment? ')
            if(procude){
                const url = (`https://intense-waters-45078.herokuapp.com/comments/${id}`)
                fetch(url,{
                    method:'DELETE',
                    headers:{
                        'content-type':'application/json'
