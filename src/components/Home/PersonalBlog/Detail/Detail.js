...
const Detail = () => {
    ...
    const {user} = useAuth()
        ...
        useEffect(()=>{
            fetch('https://intense-waters-45078.herokuapp.com/personalblog')
            .then(res => res.json())
    ...
    const handleForm = e =>{
        ...
        fetch('https://intense-waters-45078.herokuapp.com/blogComments',{
            method:'POST',
            headers:{
    ...
    const deleteComments = id =>{
        const procude = window.confirm('are you sure,you want to delete this comment? ')
            if(procude){
                const url = (`https://intense-waters-45078.herokuapp.com/blogComments/${id}`)
                fetch(url,{
                    method:'DELETE',
                    headers:{
                        'content-type':'application/json'
